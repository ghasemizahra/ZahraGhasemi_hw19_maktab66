    ...
    <header className={`header ${theme === THEME_TYPE.DARK && "header-dark"}`}> 
       <div className='d-flex justify-between align-center container'>
          ...
          <div className="theme-btn">
             {theme === THEME_TYPE.DARK ? (<BsFillMoonFill className="theme-icon" />) : (<BsSunFill className="theme-icon" />)}
              <span className={"switch-btn h-content"} onClick={handleToggle}>{theme === THEME_TYPE.DARK ? "LightMode" : "DarkMode"}</span>
            </div>
